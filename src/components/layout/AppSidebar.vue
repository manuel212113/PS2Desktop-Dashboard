                  ...
                  <div v-show="isSubmenuOpen(groupIndex, index) && (isExpanded || isHovered || isMobileOpen)">
                    <ul class="mt-2 space-y-1 ml-9">
                      <li v-for="subItem in item.subItems" :key="subItem.name">
                        <router-link
                          :to="subItem.path"
                          :class="['menu-dropdown-item', { 'menu-dropdown-item-active': isActive(subItem.path), 'menu-dropdown-item-inactive': !isActive(subItem.path) }]"
                        >
                          {{ subItem.name }}
...
import {
  GridIcon, CalenderIcon, UserCircleIcon, ChevronDownIcon, HorizontalDots, PageIcon, PlugInIcon, FolderIcon, TableIcon, LogoutIcon, BarChartIcon, UserGroupIcon, PlusIcon
} from "../../icons";
